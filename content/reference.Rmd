...
Link to page: [test](/reference/test)
...
Note: overhead modifications on long terms look bad. Instead of `\hat{\text{A very long term}}`: $\hat{\text{A very long term}}$ or `\bar{\text{A very long term}}`: $\bar{\text{A very long term}}$, try `\widehat{\text{A very long term}}`: $\widehat{\text{A very long term}}$ and `\overline{A very long term}`: $\overline{\text{A very long term}}$.

Note 2: very large symbols, such as the summation operator, may be squished if used inline (as opposed to a centered equation). To prevent this when in-line, insert `\displaystyle` in front of it. That's the difference betweeen `\sum^{n}_{i=1}`: $\sum^{n}_{i=1}$ and $\displaystyle \sum^{n}_{i=1}$.
...
|------|------|-------------|------|
| `ggplot2` | Plotting | For nice plots | [[1.3](/class/1.2-class)] | 
| `haven` | Data Wrangling | For importing nonstandard data files | | 
| `dplyr` | Data Wrangling | For manipulating data (part of tidyverse) | |
| `readr` | Data Wrangling | For importing most data files | | 
| `tidyr` | Data Wrangling | For reshaping data (wide and long) | |
| `magrittr` | Data Wrangling | For the pipe | |
| `tibble` | Data Wrangling | For a friendlier data.frame | | 
